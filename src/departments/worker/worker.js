...
export class Worker {
  createNewWorkerGroup = ({ broker, channel, workerGroupCount, processName, processPath, queueName }) => {
    // eslint-disable-next-line no-async-promise-executor
    return new Promise(async (resolve, reject) => {
      if (isMainThread) {
        const workerPath = `${processPath}.js`
        if (await this.workerExists(workerPath)) {
          const workerData = { workerGroupCount, processName, processPath, queueName }
  ...
  startWork = ({ worker, workerData, broker, channel }) => {
    ...
    return new Promise(async (resolve) => {
      await broker.consumeQueue(
        {
          channel,
          queueName: workerData.queueName,
          workerGroupCount: workerData.workerGroupCount
        }, async (messages) => {
